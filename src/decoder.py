...
def make_train_test_sets(masks, X, seed, min_trials=10, return_indices=False):
    X_train_parts = []
    y_train_parts = []
    X_test_parts = []
    y_test_parts = []
    ...
    for contrast, mask in masks.items():
        ...
        train_index_parts.append(train_idx)
        test_index_parts.append(test_idx)
    ...
    train_indices = np.concatenate(train_index_parts, axis=0)
    test_indices = np.concatenate(test_index_parts, axis=0)
    ...
    if return_indices:
        # needed by callers that must trace predictions back to specific
        # trials (e.g. saving train/test trial indices alongside results)
...
def ridge_regression(X_train, y_train, X_test, y_test, model, return_extras=False):
    ...
    if return_extras:
        # scaler is returned so callers can recover the raw-firing-rate-
        # space axis (w_raw = w / scaler.scale_) or reuse X_train_std /
        # X_test_std downstream (e.g. directional_r2_profile) without
        # re-fitting a second StandardScaler.
        return balanced_test_mse, test_r2, w, X_train_std, X_test_std, scaler
